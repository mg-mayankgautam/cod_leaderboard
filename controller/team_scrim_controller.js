...
module.exports.addTeamToScrim = async (req, res) => {
   ...
   const team = await teamDB.findOne({scrimname,teamname});
    ...
    let newTeam = new teamDB ({scrimname,teamname,team_wins,team_position_pts, played_matches, team_total_kills, team_total_pts});
    newTeam.save()
     .then(()=>{
...
module.exports.addTeamEntries=async (req, res) => {
  ...
  teamDB.updateOne({scrimname,teamname},{team_wins, team_position_pts, played_matches, team_total_pts})
  .then(()=>{
...
module.exports.removeTeamFromScrim= async (req,res) =>{
  console.log("deleteteamscrim reached");
    const {scrimname, teamname} = req.body;
    try{
        await teamDB.deleteOne({scrimname:scrimname, teamname:teamname});
